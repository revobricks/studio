...
type: string | undefined = "my-dashboard";
...
projectName: "Revobricks",
description: "Start your new Dashboard project development with local template.",
projectFileUrl: undefined
}
...
// 处理本地模板 - 新增的本地模板加载逻辑
if (this.section == "templates" && this.type == "my-dashboard") {
try {
// 构建本地模板路径 - 使用绝对路径
const localTemplatePath = path.resolve(
process.cwd(),
"packages/templates/my-dashboard/project.eez-project"
);
console.log("Loading local template from:", localTemplatePath);

// 检查文件是否存在
if (!fs.existsSync(localTemplatePath)) {
throw new Error(`Local template file not found: ${localTemplatePath}`);
}

// 从本地文件读取模板
return await readJsObjectFromFile(localTemplatePath);
} catch (err) {
console.error("Failed to load local template:", err);
throw err;
}
}
...
// 对于本地模板，确保目录存在
if (this.type == "my-dashboard") {
try {
await fs.promises.mkdir(path.dirname(this.projectFilePath!), {
recursive: true
});
} catch (err) {
runInAction(() => {
